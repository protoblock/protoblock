...
OrdersTableCancelButton::OrdersTableCancelButton(QObject *parent, QWidget *widget)
...
void OrdersTableCancelButton::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    //if(parentTable&&!mainWindow.currentlyAddingOrders)
	{
		...
		if(!buttonCancel)
		{
            ...
            buttonCancel->setFixedSize(22,22);
            connect(buttonCancel,SIGNAL(clicked()), parentWidget,SLOT(cancelOrderByXButton()));
			parentTable->setIndexWidget(index, buttonCancel);
        }

  //      buttonCancel->setProperty("OrderId",index.
