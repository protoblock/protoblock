...
/*
class QQmlObjectTreeModelBase : public QAbstractItemModel { // abstract Qt base class
    Q_OBJECT
    Q_PROPERTY (int count READ count NOTIFY countChanged)

public:
    explicit QQmlObjectTreeModelBase (QObject * parent = Q_NULLPTR) : QAbstractListModel (parent) { }
...
template<class ItemType, class ChildType> class QQmlObjectTreeModel : public QQmlObjectTreeModelBase {
public:
    explicit QQmlObjectListModel (QObject *          parent      = Q_NULLPTR,
                                  const QByteArray & displayRole = QByteArray (),
                                  const QByteArray & uidRole     = QByteArray ())
