...
namespace {
    ...
    void printNameList(const char *listName, const SshNameList &list)
    {
        ...
        foreach (const QByteArray &name, list.names)
            qDebug(sshLog, "%s", name.constData());
    }

    void printData(const char *name, const QByteArray &data)
    {
        qDebug(sshLog, "The client thinks the %s has length %d and is: %s", name, data.count(),
...
{
    qDebug(sshLog, "server requests key exchange");
    ...
    qDebug(sshLog, "First packet follows: %d", kexInitParams.firstKexPacketFollows);
