...
QString SortFilterProxyModel::sortByRoleName() const
{
    return m_sortByRoleName;
}

void SortFilterProxyModel::setSortByRoleName(const QString &sortByRoleName)
{
    if (m_sortByRoleName != sortByRoleName) {
        m_sortByRoleName = sortByRoleName;
        QByteArray ba = sortByRoleName.toLatin1 ();
        if (ba != m_sortByRoleNameArray){
            m_sortByRoleNameArray = ba;
            if (m_complete)
                QSortFilterProxyModel::setSortRole(roleKey(m_sortByRoleNameArray));
        }
    }
}
...
{
    QSortFilterProxyModel::setSortRole ( sourceModel ()->roleNames().key( role.toLatin1() ));
    QSortFilterProxyModel::sort (0 ,order);
}
...
//            when I set this is is not setting the roleName to sort by /.....
    if(!m_sortByRoleName.isEmpty ()){
        QByteArray mStringtoByte = m_sortByRoleName.toLatin1 ();
        QSortFilterProxyModel::setSortRole(roleKey(mStringtoByte));
    }
}
