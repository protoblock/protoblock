    ...
    : QObject(parent), d(new TorrentClientPrivate(this))
{
    ...
    connect(&d->fileManager, &FileManager::dataRead,            this, &TorrentClient::sendToPeer);
    connect(&d->fileManager, &FileManager::verificationProgress,this, &TorrentClient::updateProgress);
    connect(&d->fileManager, &FileManager::verificationDone,    this, &TorrentClient::fullVerificationDone);
    connect(&d->fileManager, &FileManager::pieceVerified,       this, &TorrentClient::pieceVerified);
    connect(&d->fileManager, &FileManager::error,               this, &TorrentClient::handleFileError);

    // Connect the tracker client
    connect(&d->trackerClient, &TrackerClient::peerListUpdated, this, &TorrentClient::addToPeerList);
    connect(&d->trackerClient, &TrackerClient::stopped,         this, &TorrentClient::stopped);
}
...
QList<TorrentPeer *> TorrentClient::weighedFreePeers() const
{
    ...
    for (auto peer : freePeers) {
        ...
        if (peer->interesting) {
            ...
            // 1/5 of the total score for each second below 5 it takes to connect.
...
{
    connect(client, &PeerWireClient::connected,     this, &TorrentClient::setupOutgoingConnection);
    connect(client, &PeerWireClient::disconnected,  this, &TorrentClient::removeClient);
    connect(client, &PeerWireClient::aboutToClose,  this, &TorrentClient::removeClient);
    connect(client, &PeerWireClient::piecesAvailable,this,&TorrentClient::peerPiecesAvailable);
    connect(client, &PeerWireClient::blockRequested,this, &TorrentClient::peerRequestsBlock);
    connect(client, &PeerWireClient::blockReceived, this, &TorrentClient::blockReceived);
    connect(client, &PeerWireClient::choked,        this, &TorrentClient::peerChoked);
    connect(client, &PeerWireClient::unchoked,      this, &TorrentClient::peerUnchoked);
    connect(client, &PeerWireClient::bytesWritten,  this, &TorrentClient::peerWireBytesWritten);
    connect(client, &PeerWireClient::bytesReceived, this, &TorrentClient::peerWireBytesReceived);
}
...
void TorrentClient::removeClient()
{
    ...
    disconnect(client, &PeerWireClient::disconnected, this, &TorrentClient::removeClient);
    client->deleteLater();
    ConnectionManager::instance()->removeConnection(client);
...
void TorrentClient::peerPiecesAvailable(const QBitArray &pieces)
{
    ...
    for (int pieceIndex = 0; pieceIndex < piecesSize; ++pieceIndex) {
        ...
        if (!d->completedPieces.testBit(pieceIndex)) {
            ...
            int inProgress = 0;
            auto payloadsClientsList = d->payloads.values(client);
            for ( auto client : payloadsClientsList ){
                if ( client->IsInProgress )
                    inProgress += client->RequestedBlocks.count(true);
            }
            if ( !inProgress )
                d->callScheduler();
            break;
...
void TorrentClient::blockReceived(int pieceIndex, int begin, const QByteArray &data)
{
    ...
    if (d->state != Downloading && d->state != Endgame &&
        d->completedPieces.count(true) > 0 && d->state != Paused)
    {
        d->setState(Downloading);
    }
    ...
    if (blocksLeftForPiece(piece) == 0) {
        ...
        QMultiMap<PeerWireClient *, TorrentPiece *>::Iterator it = d->payloads.begin();
        while (it != d->payloads.end()) {
            if ( it.value() == nullptr || it.value()->Index == piece->Index )
                it = d->payloads.erase( it );
            else
                ++it;
        }
...
void TorrentClient::scheduleUploads()
{
    ...
    while ( it.hasPrevious() ) {
        ...
        bool interested = (client->peerWireState() & PeerWireClient::IsInterestedPeer);
...
void TorrentClient::schedulePieceForClient(PeerWireClient *client)
{
    ...
    auto it = d->payloads.find(client);
    while (it != d->payloads.end() && it.key() == client) {
        lastPendingPiece = it.value();
        if (lastPendingPiece->IsInProgress) {
    ...
    if (d->state == WarmingUp && d->pendingPieces.size() >= 4) {
        ...
        if (!piece) {
            auto values = d->pendingPieces.values();
            piece = values.value(qrand() % values.size());
            piece->IsInProgress = true;
    ...
    if (!piece) {
        ...
        if ( (d->state == Endgame && client->uploadSpeed() < 1024) || d->state != WarmingUp) {
            for( auto it = d->pendingPieces.constBegin(); it != d->pendingPieces.constEnd(); ++it )
        ...
        while (it != d->pendingPieces.constEnd()) {
            ...
            if (incompletePiecesAvailableToClient.testBit(it.key())) {
                if (!tmp->IsInProgress || d->state == WarmingUp || d->state == Endgame) {
                    partialPieces << tmp;
                    break;
                }
            }
...
void TorrentClient::requestMore(PeerWireClient *client)
{
    ...
    while (it != d->payloads.end() && it.key() == client) {
        TorrentPiece *piece = it.value();
        if (piece->IsInProgress || (d->state == WarmingUp || d->state == Endgame))
