    ...
    : QMainWindow(parent), quitDialog(0), saveChanges(false)
{
    ...
    auto newTorrentAction = new QAction(QIcon(":/icons/bottom.png"), tr("Add &new torrent"), this);
    ...
    // Top toolbar
    auto topBar = new QToolBar(tr("Tools"));
    ...
    // Bottom toolbar
    auto bottomBar = new QToolBar(tr("Rate control"));
                            ...
                            const QByteArray &resumeState)
{
    ...
    connect(client, SIGNAL(stopped()), this, SLOT(torrentStopped()));
    connect(client, SIGNAL(sigError(TorrentClient::Error)), this, SLOT(torrentError(TorrentClient::Error)));
...
void MainWindow::about()
{
    ...
    auto quitButton = new QPushButton("OK");
...
void MainWindow::acceptFileDrop(const QString &fileName)
{
    ...
    auto addTorrentDialog = new AddTorrentDialog;
    lastDirectory = QFileInfo(fileName).absolutePath();
    addTorrentDialog->setTorrent(fileName);
    addTorrentDialog->deleteLater();
    if (!addTorrentDialog->exec())
