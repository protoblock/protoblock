...
//        anchors.right: usersList.left
        model: bufferModel
        delegate: TextBrowser {
            height: page.height - ( topicLabel.height  * 1.07)
            width: page.width - bufferListView.width  -  (usersList.width* 1.07)
            anchors{
    ...
    TextEntry {
        id: textEntry
        width:page.width - bufferListView.width  -  (usersList.width* 1.1)
        anchors{
            right: usersList.left
            rightMargin: ProtoScreen.guToPx(.5)
            bottom: messageArea.bottom
